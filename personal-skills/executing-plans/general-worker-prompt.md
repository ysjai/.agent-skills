...
允许修改的文件/符号范围：[OWNED_SCOPE；不包含 plan、TodoList、run state 或 Git 提交]
...
- 只能修改 OWNED_SCOPE 列出的文件或符号；范围可以包含源代码、测试、文档、配置和必要的生成文件。验证产生的缓存或临时输出应写入项目约定或隔离临时目录；新的持久生成文件必须先在范围中声明。
