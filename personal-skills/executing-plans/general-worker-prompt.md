...
Worker 状态路径：[WORKER_STATE_PATH]
项目局部指令：[APPLICABLE_PROJECT_INSTRUCTIONS]

边界：
- 只实现本 Task，不处理其他 Task，不扩大范围。
- 只能修改“允许修改的完整文件范围”中的实现文件；此外只允许写指定的 `Worker 状态路径`。发现其他越界需求时停止并报告主 agent。
...
开始后第一件事：核对现有 `dispatching` lease 的 Dispatch ID、Task 和 `task_manifest` 与本 Task 一致；在任何实现文件修改、外部副作用或验证命令之前，把状态更新为 `started` 并保留 lease 中的 Dispatch ID、Task、Wave、`task_manifest` 和时间（先写同目录临时文件，再 rename）。lease 缺失或内容不匹配时停止，不得修改任何实现状态。完成或阻塞时，再用下面的完整报告原子替换，然后原样回传。报告必须保留 Dispatch ID 和原始 `task_manifest`，第一项必须是 `status: completed` 或 `status: blocked`：
