...
这是 `explore-review` 模式使用的 canonical 提示词，用于在进入计划阶段前让独立 reviewer 检查设计文档。
...
Task tool（explore subagent，默认只审核一轮）：
  ...
  prompt: |
    ...
    **头脑风暴决策摘要：** [DECISION_SUMMARY；包含关键问题、用户回答、方案、取舍和结论]
    ...
    | 对话覆盖 | 对照所提供的决策摘要，文档是否捕捉关键问题、用户回答、方案、取舍和设计决策；不要推断未提供的对话 |
