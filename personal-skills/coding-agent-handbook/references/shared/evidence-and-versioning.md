...
本文件说明候选 handbook 如何区分事实、建议、版本、审批与发布。它解释资料模型，不重复任何供应商产品行为；工具事实只存在于各自的不可变 record。
...
| `evidence` | 与单个 source 对应的人工核对快照，记录定位、访问时间、核对范围和摘要哈希。 | 候选期间位于 `evidence/`；批准后才可映射到 `sources/evidence/`。 |
| `record` | 带 frontmatter 的版本化学习条目，包含事实或明确分类的建议。 | 发布后字节不可变；事实变更要创建新 record。 |
...
本候选的共享工程方法属于 `experimental-guidance`，因为没有登记第三方实践来源。它们不能被回答为供应商规定、广泛已证实的成熟实践或默认政策。
...
Qoder 当前候选的路由标签是 `unversioned-docs-2026-07-29`。它仅指向 2026-07-29 的官方在线文档快照，不是 Qoder 供应商发布的产品版本、稳定版承诺或生命周期信息。使用 Qoder 条目前必须先确认实际客户端和版本；在本候选范围外需要重新核对官方资料。
