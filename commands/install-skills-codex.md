---
description: 安装 skills 到 Codex（项目级或全局，来源支持 SkillHub、Git repo、本地 personal-skills）
---

# 安装 Skills 到 Codex

你的任务是帮用户安装 skill，以软链接方式挂载到 Codex 的指定目录。支持三种来源：SkillHub、personal-skills、git-repo-skills。

**路径约定**：
- `AGENT_SKILLS_DIR="$HOME/.agent-skills"`
- `PLATFORM="Codex"`
- `SKILLS_DIR` = `$(pwd)/.agents/skills` 或 `$HOME/.agents/skills`

> **Codex 最新配置方式**：官方文档当前推荐通过 `.agents/skills/`（项目级）和 `~/.agents/skills/`（用户级）进行原生 skill 发现，而不是旧的 `~/.codex/skills/` 路径。
...
请选择安装目标：
```
- 选项1（默认）：当前项目（`$(pwd)/.agents/skills/`，仅当前项目生效）
- 选项2：全局（`~/.agents/skills/`，所有项目均生效）
...
如果本地有已下载的 SkillHub skills，询问：直接安装 / 更新后安装 / 下载新技能。
...
列出成功安装/更新、跳过、失败的 skills。

- Codex 通常会自动发现新 skill；若未出现，再提醒用户**重启 Codex**。
- 提醒用户可在对话中输入 `$<skill-name>` 显式调用，也可用 `/skills` 查看当前已发现的 skills。
...
- 使用 Codex 官方当前推荐的原生发现目录：项目级 `$(pwd)/.agents/skills/`，全局 `$HOME/.agents/skills/`。
...
- **额外说明**：本命令只负责安装 skill 目录本身，不自动改写 Codex 的 `~/.codex/config.toml`、`hooks.json` 或第三方仓库自带的 `~/.codex/prompts` 配置；若某仓库提供这些可选增强配置，安装完成后提示用户按该仓库说明自行决定是否启用。
...
> Codex 技能通过 `.agents/skills/` 或 `~/.agents/skills/` 原生发现，可在对话中输入 `$<skill-name>` 显式调用，也可依赖 `description` 自动匹配。
