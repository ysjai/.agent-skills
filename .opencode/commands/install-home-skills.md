---
description: 安装 skills 到全局目录（~/.config/opencode/skills/），在所有项目中生效
---

# 安装全局 Skills

你的任务是帮用户安装 skills 到 `~/.config/opencode/skills/` 目录，这样在任何项目中启动 OpenCode 都可以使用这些 skills。支持三种来源：
...
- `GLOBAL_SKILLS_DIR="$HOME/.config/opencode/skills"` — 全局安装目标，软链接存放处
...
SKILL_SLUG="<slug>"
LINK_PATH="$GLOBAL_SKILLS_DIR/$SKILL_SLUG"
...
列出成功安装/更新、跳过、失败的 skills，提醒用户：**重启 OpenCode 后全局 skills 生效，在任何项目中均可使用。**
...
如果 `local-skills/` 不存在或为空，告知用户将 skill 目录（含 `SKILL.md`）手动放入 `~/.agent-skills/local-skills/` 后重试，终止流程。

### B2. 展示并选择

检查 `$HOME/.config/opencode/skills/` 已安装情况，用 question 工具展示：
...
LINK_PATH="$GLOBAL_SKILLS_DIR/<skillname>"
if [ ! -L "$LINK_PATH" ]; then
...
列出安装结果，提醒用户：**重启 OpenCode 后全局 skills 生效，在任何项目中均可使用。**
...
检查 `$HOME/.config/opencode/skills/` 已安装情况，用 question 工具展示供选择（支持整组或单独安装）：

```
请选择要安装的 skills（输入编号，多个用空格分隔，all=全选，exit=取消）：

[1] anthropics-skills 整组（17 个 skills）[部分已安装]
    [1a] frontend-design [已全局安装]
    [1b] skill-creator
    ...
